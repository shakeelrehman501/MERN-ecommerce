      ...
      {productData.productImg.length > 0 && (
        <div className="grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3">
          {productData?.productImg?.map((file, idx) => {
            ...
            return (
              <Card key={idx} className="relative group/card overflow-hidden">
                <CardContent>
                  <img
                    src={preview}
                    alt=""
                  ...
                  <button
                    ...
                    className="absolute top-2 z-10 right-2 bg-black/50 text-white p-1 rounded-full cursor-pointer opacity-100 hover:opacity-80 hover:cursor-pointer  transition"
                  >
                    <X size={14} />
                  </button>
                </CardContent>
